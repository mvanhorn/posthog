...
pub async fn validate_secret_api_token_for_team(
    ...
    let result = state
        .auth_token_cache
        .get_or_load(&token_hash, |_key| async move {
            // Try Team.secret_api_token / secret_api_token_backup first
            match Team::from_pg_by_secret_token(pg_reader.clone(), &token_owned).await {
                Ok(team) => {
                    return Ok::<_, FlagError>(Some(TokenAuthData::Secret { team_id: team.id }))
                }
                Err(_) => {} // Not a team secret token, try ProjectSecretAPIKey
            }
    ...
    match result.value {
        Some(TokenAuthData::Secret { team_id }) if team_id == expected_team_id => {
            ...
            Ok(TokenAuthData::Secret { team_id })
        }
        Some(TokenAuthData::ProjectSecret { team_id, .. }) if team_id == expected_team_id => {
